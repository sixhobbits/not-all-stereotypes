...
clean.data <- function()
{
     ...
     d1 <- read.table('data.raw.txt', sep='\t', header=T, quote='')
                      #as.is=T)   # don't convert column classes)  # 49708 x 28
     ...
     # get all subs info
     allSubs <- subset(d1, Item==1 & Word.number==1)  # 32 x 28
     allSubs <- allSubs[ , c("Subj","Age","Consent","Gender","NativeGerman","DesignNum","IP","Date")] # 32 x 8
     length(unique(allSubs$Subj)) # 31  --  doubles of 00
     write.table(allSubs, 'all.subs.txt', sep="\t", row.names=F, quote=F)  
     
     # remove "test" data
     d1 <- subset(d1, Subj != "00" &
                       Subj != "111111" &
                       Subj != "Vojtech_test")  # 45084 x 28
     
     # replace "46_newTry" with "46"
     # convert Subj col to character class
     d1$Subj <- as.character(d1$Subj)
     #d1$Subj[d1$Subj=="46_newTry"]  <- "46"
     d1[d1=="46_newTry"] <- "46"    # Gareth's method
     
     # remove practice trials
     prac <- c("p1","p2","p3","p4")
     d1 <- subset(d1, !Cond %in% prac) # 43095 x 28
     
     # get CW position from file that was output by stims.to.ibex.format.R
     cw.num <- read.table("CW.number.txt", sep="\t", header=T, quote='')
     d1$CW.number <- NA  # add col    # 30940 x 29
     ...
     for (s in subs) # for each subject
     {
          for (i in items) # for each item
          {
               ...
               # adding cw.p2 (Note: Items 5,7,9,23 only have 1 word following cw)
               d1$Word.number[d1$Subj==s & d1$Item==i & d1$Controller=='sentence' &      
                                   d1$Word.number == cw+2 ] <- "cw.p2"
          }
     }  # 30940 x 29
     ...
     d2 <- subset(d2, Controller=='sentence') # 28140 x 31
     ...
     # mark repeats in d2 -- IN OUR CASE, REPEATS ARE OK, BECAUSE PARTICIPANTS ARE USING OUR COMPUTERS
     d2$Repeat <- 0
     d2$Repeat[d2$Subj %in% repeats] <- 1  # 8040
     ...
     d2 <- subset(d2, select = -c(IbexSubj,IP,Controller,Foo,Element)) # 28140 x 27
     

     # write table to show order of trials
     trials <- subset(d2, Word.number==1)
     write.table(trials, 'order.of.trials.txt', sep="\t", row.names=F, quote=F)  # 2800 x 27
     
     # write clean data
     write.table(d2, 'data.clean.txt', sep="\t", row.names=F, quote=F)  # 39195 x 27
